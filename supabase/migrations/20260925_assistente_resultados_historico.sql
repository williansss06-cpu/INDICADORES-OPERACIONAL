...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
      ...
      'meta', q.meta,
      'polaridade', q.polaridade
    ) ORDER BY q.mes, q.ordem, q.indicador_id), '[]'::jsonb)
    INTO v_resultados_historico
  FROM (
    SELECT
      r.id,
      r.operacao_id,
  ...
  v_base := jsonb_set(v_base, '{resultados_historico}', v_resultados_historico, true);
  RETURN v_base;
END;
$$;
