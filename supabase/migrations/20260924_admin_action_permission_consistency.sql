-- Corrige a consistência das permissões de Plano de Ação.
-- Qualquer ação operacional exige também Visualizar, conforme a constraint da tabela.
-- A normalização é feita no backend para proteger chamadas diretas à RPC.
...
BEGIN
  ...
  SELECT p_user_id,x.operacao_id,(coalesce(x.pode_visualizar,false) OR coalesce(x.pode_criar,false) OR coalesce(x.pode_editar,false) OR coalesce(x.pode_alterar_status,false) OR coalesce(x.pode_concluir,false) OR coalesce(x.pode_excluir,false)),coalesce(x.pode_criar,false),coalesce(x.pode_editar,false),coalesce(x.pode_alterar_status,false),coalesce(x.pode_concluir,false),coalesce(x.pode_excluir,false),true,now()
